...
export default function GirlsReceiptsPage() {
  ...
  const handleDelete = async () => {
    try {
      await deleteDoc(doc(db, "girls_receipts", deleteDialog.receipt.id));
  ...
  const handleBulkDelete = async () => {
    try {
      ...
      selected.forEach((id) => batch.delete(doc(db, "girls_receipts", id)));
      await batch.commit();
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        title="Receipts"
        description="Manage fee receipts for girl students"
        actions={
          <Link
            href="/girls/receipts/add"
          ...
          >
            <div className="flex items-center gap-3">
              <div
                ...
                style={{ background: T.hover, color: T.green }}
              >
                {icon}
              </div>
        ...
        <div className="flex flex-col md:flex-row gap-3">
          ...
          <div className="flex gap-2">
            <button
              ...
              style={{
                borderColor:
                  showFilters || activeFilters > 0 ? T.green : T.border,
                background:
                  showFilters || activeFilters > 0 ? "#E8F5EE" : "white",
                color: showFilters || activeFilters > 0 ? T.green : T.text,
              }}
            >
      ...
      {selected.length > 0 && (
        <div
          ...
          style={{ background: "#E8F5EE", border: `1px solid ${T.green}` }}
        >
          ...
          <div className="flex gap-2">
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium text-white rounded"
              ...
              >
                <tr>
                  ...
                  {[
                    ...
                    <th
                      key={h}
                      className={`px-4 py-3 text-xs font-semibold uppercase tracking-wide ${h === "" ? "text-right" : "text-left"}`}
                      style={{ color: T.muted }}
                    >
                      {h}
                    </th>
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-1">
                        <Link
                          href={`/girls/receipts/${r.id}`}
                          style={{ color: T.muted }}
                          className="p-1.5 rounded hover:bg-[#F5EFE8]"
                        >
                          <Eye size={15} />
                        </Link>
                        <Link
                          href={`/girls/receipts/${r.id}/print`}
                          style={{ color: T.muted }}
                          className="p-1.5 rounded hover:bg-[#F5EFE8]"
                        >
                          <Printer size={15} />
                        ...
                        <button
                          onClick={() =>
                            setDeleteDialog({ open: true, receipt: r })
                          }
                          style={{ color: T.muted }}
      ...
      <ConfirmDialog
        ...
        description={`Delete receipt ${deleteDialog.receipt?.receiptNumber}? This cannot be undone.`}
        confirmText="Delete"
        variant="danger"
      />
      <ConfirmDialog
        ...
        description={`Delete ${selected.length} receipts? This cannot be undone.`}
