  ...
  return (
    <div>
      <div className="print:hidden mb-6 flex items-center justify-between">
        ...
        <button onClick={handlePrint} className="inline-flex items-center gap-2 px-4 py-2 bg-pink-600 text-white rounded-lg hover:bg-pink-700"><Printer className="h-4 w-4" />Print Receipt</button>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-8 print:border-none print:rounded-none print:p-0 max-w-3xl mx-auto">
        <div className="text-center border-b-2 border-gray-800 pb-4 mb-6">
          <h1 className="text-2xl font-bold text-gray-900 uppercase tracking-wide">Girls Hostel</h1>
          <p className="text-gray-600 mt-1">Student Management System</p>
          <p className="text-gray-500 text-sm mt-1">123 Education Street, City - 123456</p>
      ...
      <style jsx global>{`@media print { body * { visibility: hidden; } .print\\:hidden { display: none !important; } #__next > div > div:last-child, #__next > div > div:last-child * { visibility: visible; } #__next > div > div:last-child { position: absolute; left: 0; top: 0; width: 100%; } }`}</style>
    </div>
  );
}
